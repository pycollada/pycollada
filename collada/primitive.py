...
class Primitive(DaeObject):
    ...
    vertex = property(
        lambda s: s._vertex,
        doc="""Read-only numpy.array of size Nx3 where N is the number of vertex points in the
    primitive's vertex source array.""",
    )
    normal = property(
        lambda s: s._normal,
        doc="""Read-only numpy.array of size Nx3 where N is the number of normal values in the
    primitive's normal source array.""",
    )
    texcoordset = property(
        lambda s: s._texcoordset,
        doc="""Read-only tuple of texture coordinate arrays. Each value is a numpy.array of size
    Nx2 where N is the number of texture coordinates in the primitive's source array.""",
    )
    textangentset = property(
        lambda s: s._textangentset,
        doc="""Read-only tuple of texture tangent arrays. Each value is a numpy.array of size
    Nx3 where N is the number of texture tangents in the primitive's source array.""",
    )
    texbinormalset = property(
        lambda s: s._texbinormalset,
        doc="""Read-only tuple of texture binormal arrays. Each value is a numpy.array of size
    Nx3 where N is the number of texture binormals in the primitive's source array.""",
    )

    vertex_index = property(
        lambda s: s._vertex_index,
        doc="""Read-only numpy.array of size Nx3 where N is the number of vertices in the primitive.
    ...
    normal_index = property(
        lambda s: s._normal_index,
        doc="""Read-only numpy.array of size Nx3 where N is the number of vertices in the primitive.
    To get the actual normal values, one can use this array to select into the normals
    array, e.g. ``normal[normal_index]``.""",
    )
    texcoord_indexset = property(
        lambda s: s._texcoord_indexset,
        doc="""Read-only tuple of texture coordinate index arrays. Each value is a numpy.array of size
    ...
    textangent_indexset = property(
        lambda s: s._textangent_indexset,
        doc="""Read-only tuple of texture tangent index arrays. Each value is a numpy.array of size
    ...
    texbinormal_indexset = property(
        lambda s: s._texbinormal_indexset,
        doc="""Read-only tuple of texture binormal index arrays. Each value is a numpy.array of size
    Nx3 where N is the number of vertices in the primitive. To get the actual texture
    ...
    def _getInputsFromList(collada, localscope, inputs):
                ...
                if isinstance(vertex_source, dict):
                    for inputsemantic, inputsource in vertex_source.items():
                        if inputsemantic == "POSITION":
                            to_append.append(
                                [offset, "VERTEX", "#" + inputsource.id, set]
                            )
                        else:
                            to_append.append(
                                [offset, inputsemantic, "#" + inputsource.id, set]
                            )
        ...
        for input in inputs:
            ...
            if len(source) < 2 or source[0] != "#":
                raise DaeMalformedError(f'Incorrect source id "{source}" in input')
            if source[1:] not in localscope:
                raise DaeBrokenRefError(f'Source input id "{source}" not found')
            ...
            if semantic == "VERTEX":
                ...
                try:
                    raise DaeUnsupportedError(f"Unknown input semantic: {semantic}")
    ...
    def _getInputs(collada, localscope, inputnodes):
        try:
            inputs = [
                (int(i.get("offset")), i.get("semantic"), i.get("source"), i.get("set"))
                for i in inputnodes
            ]
        except ValueError:
    ...
    def getInputList(self):
        ...
        inpl = InputList()
        for _key, tupes in self.sources.items():
            for offset, semantic, source, set, _srcobj in tupes:
