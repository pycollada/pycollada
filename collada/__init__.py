...
from collada.util import BytesIO, IndexedList, basestring
from collada.xmlutil import createElementTree, writeXML
...
class Collada:
    ...
    def __init__(
        ...
        try:
            ...
            raise DaeMalformedError(f"XML Parsing Error: {e}")
    ...
    def _getFileFromZip(self, fname):
        ...
        if not self.zfile:
            raise DaeBrokenRefError(
                f"Trying to load an auxiliary file {fname} but we are not reading from a zip"
            )
        ...
        if aux_path not in self.zfile.namelist():
            raise DaeBrokenRefError(f"Auxiliar file {fname} not found in archive")
    ...
    def _getFileFromDisk(self, fname):
        ...
        if self.zfile:
            raise DaeBrokenRefError(
                f"Trying to load an auxiliary file {fname} from disk but we are reading from a zip file"
            )
        basepath = os.path.dirname(self.filename)
        aux_path = os.path.normpath(os.path.join(basepath, fname))
        if not os.path.exists(aux_path):
            raise DaeBrokenRefError(f"Auxiliar file {fname} not found on disk")
    ...
    def _wrappedFileLoader(self, aux_file_loader):
        def __wrapped(fname):
            ...
            if res is None:
                raise DaeBrokenRefError(
                    f"Auxiliar file {fname} from auxiliary file loader not found"
                )
            return res
            ...
            for libnode in libnodes:
                if libnode is not None:
                    for controlnode in libnode.findall(self.tag("controller")):
                        if (
                            controlnode.find(self.tag("skin")) is None
    ...
    def _loadDefaultScene(self):
        ...
        node = self.xmlnode.find(
            "{}/{}".format(self.tag("scene"), self.tag("instance_visual_scene"))
        )
        try:
            if node is not None:
                ...
                if not sceneid.startswith("#"):
                    raise DaeMalformedError(
                        f"Malformed default scene reference to {sceneid}: "
                    )
                self.scene = self.scenes.get(sceneid[1:])
                if not self.scene:
                    raise DaeBrokenRefError(f"Default scene {sceneid} not found")
        except DaeError as ex:
            self.handleError(ex)
    ...
    def save(self):
        ...
        libraries = [
            (self.geometries, "library_geometries"),
        ...
        if self.scene is not None:
            ...
            if sceneid not in self.scenes:
                raise DaeBrokenRefError(f"Default scene {sceneid} not found")
            scenenode.append(E.instance_visual_scene(url=f"#{sceneid}"))

        if self.validator is not None:
            if not self.validator.validate(self.xmlnode):
                raise DaeSaveValidationError(
                    f"Validation error when saving: {self.validator.COLLADA_SCHEMA_1_4_1_INSTANCE.error_log.last_error.message}"
                )
