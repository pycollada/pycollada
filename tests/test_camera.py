...
class TestCamera(unittest.TestCase):
    ...
    def test_perspective_camera_xfov_yfov_aspect_ratio(self):
        ...
        with self.assertRaises(DaeMalformedError):
            cam = collada.camera.PerspectiveCamera(
                "mycam", 1, 1000, xfov=None, yfov=None, aspect_ratio=None)
        with self.assertRaises(DaeMalformedError):
            cam = collada.camera.PerspectiveCamera(
                "mycam", 1, 1000, xfov=0.2, yfov=30, aspect_ratio=50)
        with self.assertRaises(DaeMalformedError):
            cam = collada.camera.PerspectiveCamera(
                "mycam", 1, 1000, xfov=None, yfov=None, aspect_ratio=50)

        # xfov alone
        cam = collada.camera.PerspectiveCamera(
            "mycam", 1, 1000, xfov=30, yfov=None, aspect_ratio=None)
        ...
        # yfov alone
        cam = collada.camera.PerspectiveCamera(
            "mycam", 1, 1000, xfov=None, yfov=50, aspect_ratio=None)
        ...
        #xfov + yfov
        cam = collada.camera.PerspectiveCamera(
            "mycam", 1, 1000, xfov=30, yfov=50, aspect_ratio=None)
        self.assertEqual(cam.xfov, 30)
        self.assertEqual(cam.yfov, 50)
        self.assertIsNone(cam.aspect_ratio)
        ...
        #xfov + aspect_ratio
        cam = collada.camera.PerspectiveCamera(
            "mycam", 1, 1000, xfov=30, yfov=None, aspect_ratio=1)
        self.assertEqual(cam.xfov, 30)
        self.assertIsNone(cam.yfov)
        ...
        #yfov + aspect_ratio
        cam = collada.camera.PerspectiveCamera(
            "mycam", 1, 1000, xfov=None, yfov=50, aspect_ratio=1)
        self.assertIsNone(cam.xfov)
        self.assertEqual(cam.yfov, 50)
        self.assertEqual(cam.aspect_ratio, 1)
    ...
    def test_perspective_camera_saving(self):
        ...
        cam = collada.camera.PerspectiveCamera.load(
            self.dummy, {}, fromstring(tostring(cam.xmlnode)))
        self.assertEqual(cam.id, "mycam")
        self.assertEqual(cam.znear, 1)
        self.assertEqual(cam.zfar, 1000)
        self.assertEqual(cam.xfov, 30)
    ...
    def test_orthographic_camera_xmag_ymag_aspect_ratio(self):
        ...
        with self.assertRaises(DaeMalformedError):
            cam = collada.camera.OrthographicCamera(
                "mycam", 1, 1000, xmag=None, ymag=None, aspect_ratio=None)
        with self.assertRaises(DaeMalformedError):
            cam = collada.camera.OrthographicCamera(
                "mycam", 1, 1000, xmag=0.2, ymag=30, aspect_ratio=50)
        with self.assertRaises(DaeMalformedError):
            cam = collada.camera.OrthographicCamera(
                "mycam", 1, 1000, xmag=None, ymag=None, aspect_ratio=50)

        # xmag alone
        cam = collada.camera.OrthographicCamera(
            "mycam", 1, 1000, xmag=30, ymag=None, aspect_ratio=None)
        ...
        # ymag alone
        cam = collada.camera.OrthographicCamera(
            "mycam", 1, 1000, xmag=None, ymag=50, aspect_ratio=None)
        ...
        #xmag + ymag
        cam = collada.camera.OrthographicCamera(
            "mycam", 1, 1000, xmag=30, ymag=50, aspect_ratio=None)
        self.assertEqual(cam.xmag, 30)
        self.assertEqual(cam.ymag, 50)
        self.assertIsNone(cam.aspect_ratio)
        ...
        #xmag + aspect_ratio
        cam = collada.camera.OrthographicCamera(
            "mycam", 1, 1000, xmag=30, ymag=None, aspect_ratio=1)
        self.assertEqual(cam.xmag, 30)
        self.assertIsNone(cam.ymag)
        self.assertEqual(cam.aspect_ratio, 1)
        ...
        #ymag + aspect_ratio
        cam = collada.camera.OrthographicCamera(
            "mycam", 1, 1000, xmag=None, ymag=50, aspect_ratio=1)
        self.assertIsNone(cam.xmag)
        self.assertEqual(cam.ymag, 50)
        self.assertEqual(cam.aspect_ratio, 1)
    ...
    def test_orthographic_camera_saving(self):
        ...
        cam = collada.camera.OrthographicCamera.load(
            self.dummy, {}, fromstring(tostring(cam.xmlnode)))
        self.assertEqual(cam.id, "mycam")
        self.assertEqual(cam.znear, 1)
        self.assertEqual(cam.zfar, 1000)
