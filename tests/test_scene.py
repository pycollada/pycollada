...
class TestScene(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.yourcam = collada.camera.PerspectiveCamera(
            "yourcam", 45.0, 0.01, 1000.0)
        self.dummy.cameras.append(self.yourcam)

        self.yourdirlight = collada.light.DirectionalLight(
            "yourdirlight", (1, 1, 1))
    ...
    def test_scene_matrix_node(self):
        matrix = collada.scene.MatrixTransform(numpy.array(
            [1.0, 0, 0, 2, 0, 1, 0, 3, 0, 0, 1, 4, 0, 0, 0, 1]))
    ...
    def test_scene_node_combos(self):
        ...
        justchildren = collada.scene.Node(
            'myjustchildrennode', children=[emptynode])
        ...
        scale = collada.scene.ScaleTransform(0.1, 0.2, 0.3)
        justtransform = collada.scene.Node(
            'myjusttransformnode', transforms=[scale])
        ...
        loadedboth = collada.scene.Node.load(
            self.dummy, fromstring(tostring(both.xmlnode)), {})
        self.assertEqual(len(both.children), 2)
        self.assertEqual(len(both.transforms), 1)
    ...
    def test_scene_node_saving(self):
        ...
        self.assertTrue(
            isinstance(
                yournode.transforms[0],
                collada.scene.ScaleTransform))
        self.assertTrue(
            isinstance(
                yournode.transforms[1],
    ...
    def test_scene_geometry_node(self):
        ...
        matnode = collada.scene.MaterialNode(
            "mygeommatref", self.effect, [binding])
        ...
        matnode2 = collada.scene.MaterialNode(
            "yourgeommatref", self.effect, [binding])
    ...
    def test_scene_node_with_instances(self):
        ...
        mynode = collada.scene.Node('mynode',
                                    children=[
                                        myemptynode, geomnode, camnode, lightnode],
